...
TEST_CASE("type_name")
{
	...
	{
		bool res;
		res = type_name<size_t>() == "unsigned long long";  // TODO: insert the type name
		CHECK(res);
		...
		CHECK(res);
		res = type_name<std::function<const char*(const int&)>>() == "std::function<char const* (int const&)>";  // TODO: insert the type name
		CHECK(res);
