...
int sum(T... containers){
    auto sum = (0 + ... + std::accumulate(std::begin(containers), std::end(containers), 0));
    return sum;
}
/*
template<typename T>
std::enable_if_t<is_container_v<T>, int>
sum(T&& v, T&& v2){
...
struct summation{
    template <IsContainer... T>
    int operator() (const T... containers){
        return sum(containers... );
    }
};

template <typename T, IsContainer... C>
auto lazy(T&& obj, C&&... con){
    return std::bind(std::forward<T>(obj), std::forward<C>(con)...);
}
